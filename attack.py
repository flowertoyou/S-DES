...
def brute_force_decrypt(ciphertext, known_plaintext, lock):
        for key in generate_keys():
            sdes = DECRIPTION()
            decrypted = DECRIPTION.Decryption(ciphertext)
...
if __name__ == "__main__":
    ...
    ciphertext = [1,0,0,0,0,0,0,1]
    known_plaintext = [1,0,0,0,1,1,1,0]

    lock = Lock()
    threads = []
    start_time = time.time()

    # Start multiple threads for brute force attack
    for _ in range(4):  # Number of threads
        thread = Thread(target=brute_force_decrypt, args=(ciphertext, known_plaintext, lock))
